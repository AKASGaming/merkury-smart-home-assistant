...
def _cleanup_device_links(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Remove legacy account hub and parent links from older integration versions."""
    ...
    for dev in get_entry_devices(entry):
        device = device_registry.async_get_device(
            identifiers={(DOMAIN, dev[CONF_DEVICE_ID])}
        )
        if device is not None and device.via_device_id is not None:
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _cleanup_device_links(hass, entry)
